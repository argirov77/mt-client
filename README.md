...
- **Deep-link purchase session flow (`opaque`):** Open `/api/q/{opaque}` first to establish purchase session cookies, then call `/api/public/purchase/{id}` (read) and `/api/public/purchase/{id}/pay` (mutation with `X-CSRF`). Client helpers already send `credentials: "include"` and attach `X-CSRF` from `mc_csrf` for non-GET requests.【F:src/utils/fetchWithInclude.ts†L11-L33】【F:src/components/purchase/PurchaseClient.tsx†L1026-L1034】【F:src/components/purchase/PurchaseClient.tsx†L1751-L1754}
