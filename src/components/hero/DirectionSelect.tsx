...
const fuzzyFilterStops = (stops: Stop[], query: string, maxItems = 30) => {
  ...
  return stops
    .map((stop) => {
      ...
      const fullDistance = levenshteinDistance(cleanQuery, name);
      const firstWordDistance = levenshteinDistance(cleanQuery, firstWord);
      const containsBonus = name.includes(cleanQuery) ? -1 : 0;

      return {
        stop,
        score: Math.min(fullDistance, firstWordDistance) + containsBonus,
      };
    })
...
export default function DirectionSelect({
  ...
  useEffect(() => {
    if (from && prevFromRef.current !== from) {
      setFromQuery(selectedFromName);
    }

    if (!from && prevFromRef.current && fromQuery === selectedFromName) {
      setFromQuery('');
    }

    prevFromRef.current = from;
  }, [from, selectedFromName, fromQuery]);

  useEffect(() => {
    if (to && prevToRef.current !== to) {
      setToQuery(selectedToName);
    }

    if (!to && prevToRef.current && toQuery === selectedToName) {
      setToQuery('');
    }

    prevToRef.current = to;
  }, [to, selectedToName, toQuery]);
