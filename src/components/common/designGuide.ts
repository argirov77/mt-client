export const designGuide = {
  typography: {
    fontFamily: "Inter, 'Roboto', 'Helvetica Neue', Arial, sans-serif",
    baseSize: "16px",
    headingWeight: 700,
    subheadingWeight: 600,
    bodyWeight: 400,
  },
  colors: {
    accent: "#ff6a00",
    secondary: "#0ea5e9",
    section: "#f5f7fb",
    textPrimary: "#111827",
    textSecondary: "#6b7280",
    cardBorder: "#e2e8f0",
  },
  surfaces: {
    cardRadius: "20px",
    cardShadow: "0 14px 30px rgba(15, 23, 42, 0.06)",
  },
};

export const sectionBgMuted = "bg-[#f5f7fb]";
export const headingH2Class = "text-3xl font-bold tracking-tight text-slate-900";
export const bodyTextClass = "text-base text-slate-700";
export const cardBaseClass =
  "rounded-2xl bg-white shadow-[0_14px_30px_rgba(15,23,42,0.06)] ring-1 ring-slate-200";
export const iconCircleClass =
  "grid h-10 w-10 place-items-center rounded-full bg-sky-100 text-sky-700 ring-1 ring-slate-100 shadow";
export const accentPillClass =
  "inline-flex items-center rounded-full bg-[#ff6a00]/10 px-3 py-1 text-sm font-semibold text-[#ff6a00]";
export const secondaryEyebrowClass =
  "text-xs font-semibold uppercase tracking-[0.18em] text-sky-600";
