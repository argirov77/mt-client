...
Открывать сайт с `?debug_mode=1` — GA4 debug mode включается для текущей
вкладки и сохраняется в `sessionStorage` (`ga4_debug_mode`), поэтому переживает
Next.js-навигацию, при которой query-параметр теряется. Выключить: `?debug_mode=0`.
В dev `debug_mode: true` включён всегда. Пока debug-режим активен, каждое событие
логируется в консоль как `[GA4] { eventName, params, debugMode, measurementId }`,
а в dev дополнительно как `[Analytics] …`. Параметр `debug_mode: false` не
отправляется никогда — при выключенном режиме параметр полностью исключается,
поэтому обычные production-посетители в DebugView не попадают.
